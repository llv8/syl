$(function() {
  $("#chat_textarea").bind("keypress", function(event) {
    ...
    if (event.keyCode === 27) {
      ...
      /*
       * var parents = syl.wnd.activeele.parents(); for (var i = 0; i <
       * parents.length; i++) { if ($(parents[i]).attr('tabIndex')) {
       * $(parents[i]).focus(); break; } }
       */
    ...
    source: function(request, response) {
      ...
      if (term.indexOf("@") === 0 && term.split("@").length == 2) {
        ...
        if (term.lenght == 1) {
          ...
          var filterUsers = [];
          var matcher = syl.util.single_match(term.substr(1, term.length));
