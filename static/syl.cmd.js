      ...
      source: function(request, response) {
        ...
        if (groups) {
          ...
          if (!cmd_line.match(/^applygroup\s+$/)) {
            var groupname = cmd_line
                    .substr(cmd_line.match(/^applygroup\s+/)[0].length,
                            cmd_line.length);
    ...
    var approveuser_obj = {
      source: function(request, response) {
        ...
        $.each(filterUsers, function(i, n) {
          result.push({
            label: 'approveuser ' + filterUsers[i].n + ' (' + filterUsers[i].gn
                    + ')',
      ...
      if (event.keyCode === 13) {
        ...
        if (!cmd) {
          ...
          if (err_msg) {
            ...
            if (cmd_line.match(/applygroup\s+/)) {
              var groupid = $('#cmd').autocomplete("instance").options.groupid;
              cmd_line = 'apply_group ' + groupid;
            }

            if (cmd_line.match(/approveuser\s+/)) {
              var groupid = $('#cmd').autocomplete("instance").options.groupid;
              var userid = $('#cmd').autocomplete("instance").options.userid;
              cmd_line = 'approve_user ' + groupid + ' ' + userid;
            }
