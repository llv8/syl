...
$(function() {
	...
	read_records : function(filename, read_resp, len) {
				    ...
				    fileEntry
					    .file(
							...
							reader.onloadend = function(
								e) {
							    var records = this.result
								    .split('\n');
							    var lastestrecords = records.lenght > len ? records
								    .slice(
									    records.length
										    - len,
									    records.length)
	    ...
	    window.requestFileSystem(window.TEMPORARY,
		    50 * 1024 * 1024 /* 50MB */, read_handler,
		    this.error_handler);
	},
	...
	write : function(filename, data, write_resp) {
	    ...
	    window.requestFileSystem(window.TEMPORARY,
		    50 * 1024 * 1024 /* 50MB */, write_handler,
		    this.error_handler);
	},
	write_records : function(filename, records, write_resp) {
	    this.filename = filename;
	    var write_handler = function(fs) {
		fs.root.getFile(filename, {
		    create : true
