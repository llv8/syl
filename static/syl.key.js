  ...
  syl.key = {
    ...
    key_cnf: function() {
...

      {
        ...
        'key': 'p',
        'req': th.open_close_popup,
        'help': 'open or close popup'
      },
    ...
    __focus_handler: function() {
      ...
      all_el.focus(function() {
        all_el.filter('input,textarea,div[tabIndex]').removeClass('a_b')
                .addClass('i_b');
    ...
    __cur_wnd: function() {
      var cur_wnd = null;
      var cur_el = syl.key.__cur_el();
      if ($(cur_el).hasClass('wnd')) {
        cur_wnd = $(cur_el);
      } else {
        cur_wnd = $(cur_el).parents('.wnd');
      }
      return cur_wnd;
    },
    __cur_el: function() {
    ...
    scroll_d: function() {
      $(syl.key.__cur_el()).scroll(function(event) {
        this.scrollTop = this.scrollTop + 20;
        $(this).unbind('scroll');

      });
      $(syl.key.__cur_el()).scroll();
    },
    ...
    f_line: function() {
      $(syl.key.__cur_el()).scrollTop(0);
    },

    l_line: function() {
      $(syl.key.__cur_el()).scrollTop(syl.key.__cur_el().scrollHeight);
    },
    ...
    active_el: function() {
      ...
      for (var i = 0; i < els.length; i++) {
        if ($(els[i]).css('display') == 'none' || $(els[i]).hasClass('wnd')) {
          continue;
        }
        ...
        if (top + parseInt($(els[i]).css('height').replace('px', '')) < 0
                || left + parseInt($(els[i]).css('width').replace('px', '')) < 0) {
          continue;
        }
        ...
        for (var j = 0; j < parents.length; j++) {
          ...
          if ($(parents[j]).css('position') == 'relative') {
            ...
            var p_height = parseInt($(parents[j]).css('height').replace('px',
                    ''));
            var p_width = parseInt($(parents[j]).css('width').replace('px', ''));
          ...
          Mousetrap.bind(tip_content.split('').join(' '), function(event) {
            if (els[ind].tagName.toUpperCase() == 'A') {
              ...
              if (!url || url.indexOf('#') == 0) {
                $(els[ind]).click();
                $($(els[ind]).attr('href')).focus();
              } else {
                window.open(url);
              }
            } else if (els[ind].tagName.toUpperCase() == 'INPUT'
                    || els[ind].tagName.toUpperCase() == 'DIV'
                    || els[ind].tagName.toUpperCase() == 'TEXTAREA') {
              $(els[ind]).focus();
              event.preventDefault();
            }
            syl.key.fkey_empty();
    ...
    chat_send: function(event) {
      if (event.keyCode == 64 && $('#chat_content').val() == '') {
        ...
        if (syl.touser) {
          if (!$(syl.key.__cur_el()).is($('#chat_textarea'))) return;
          var cmd = 'CHAT';
    ...
    load_notice: function() {
      ...
      var filename = 'notice-' + id;
      syl.fs.read_records(filename, function(records) {
        for ( var i in records) {
          $('#popup_content').html(
                  '<div class="notice">' + records[i] + '</div>'
                          + $('#popup_content').html());
        }
      }, 9);
    },
    open_close_popup: function() {
      if ($('#popup').hasClass('ndp')) {
        syl.key.load_notice();
        $('#popup').removeClass('ndp').addClass('dp');
        var popup = $('#popup')[0];
        popup.style.top = document.body.scrollTop - popup.clientHeight - 24
                + "px";
        popup.style.left = document.body.scrollLeft + document.body.clientWidth
                - popup.clientWidth - 4 + "px";
        syl.pre_popup_activeel = syl.key.__cur_el();
        $('#popup_content').focus();
      } else {
    ...
    max_or_min_wnd: function() {
      ...
      if (cur_wnd.attr('id') == 'popup') {
        if (cur_wnd.attr('pre-width')) {
          ...
          $(cur_wnd).css('width', document.body.clientWidth - 4);
          $(cur_wnd).css('height', document.body.clientHeight - 8);
          $(cur_wnd).css('top', -1 * (document.body.clientHeight + 14));
          $(cur_wnd).css('left', 0);
        }
        return;
      }
      if (cur_wnd.hasClass('max')) {
        ...
        var percent_height = parseInt(height.replace(/px/, ''))
                / (all_height - 20 - 8) * 100;
        syl.key.__pre_pos(list[0], 'pre-height', Math.ceil(percent_height));
        syl.key.__pre_pos(list[0], 'pre-width', Math.ceil(percent_width));
        syl.key.__pre_pos(list[1], 'pre-height', Math.ceil(percent_height));
        syl.key.__pre_pos(list[1], 'pre-width', 100 - Math.ceil(percent_width));
        syl.key.__pre_pos(list[2], 'pre-height', 100 - Math
                .ceil(percent_height));
        syl.key.__pre_pos(list[2], 'pre-width', Math.ceil(percent_width));
        syl.key.__pre_pos(list[3], 'pre-height', 100 - Math
                .ceil(percent_height));
        syl.key.__pre_pos(list[3], 'pre-width', 100 - Math.ceil(percent_width));
